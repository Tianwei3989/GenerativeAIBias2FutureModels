...
parser.add_argument(
    "--seed", type=int, default=123, help="Default random seed."
)
parser.add_argument(
    "--outdir",
    type=str,
...
for tar_file in target_tars:
    ...
    mix_ids = np.random.choice(2, size=len(files)//3, p=[1-args.mix_ratio, args.mix_ratio])  # 3 as 1 image always has its json and caption txt
    ...
    with tarfile.open(dst_path + tar_file.split('/')[-1], "w:gz") as archive:
        ...
        for i in range(len(files)):
            ...
            if files[i][-3:] == 'txt' or (files[i][-3:] in ['jpg','png','peg'] and mix_ids[file_id] != 1):
                if files[i][-3:] == 'txt':
                ...
                archive.add(args.sd_cc3m_path + str(file_id_).zfill(8) + ".png", arcname=files[i])
