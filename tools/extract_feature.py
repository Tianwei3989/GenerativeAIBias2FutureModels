...
if args.text_file is not None:
    ...
    for i in tqdm(range(len(prompts))):
        ...
        prompt_feature_dict[prompt_ids[i]] = (
            text_features.to("cpu").detach().numpy().copy()
        )
...
if args.image_file is not None:
    ...
    for image_name in tqdm(image_files):
        img = (
            preprocess(Image.open(os.path.join(file_name, image_name)))
            .unsqueeze(0)
            .to(device)
        )
        with torch.no_grad(), torch.cuda.amp.autocast():
        ...
        image_feature_dict[image_name[:-4]] = (
            image_features.squeeze().detach().to("cpu").numpy().copy()
        )
