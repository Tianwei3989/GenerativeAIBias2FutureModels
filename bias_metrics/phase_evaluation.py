...
anns_path = os.path.join(
    args.data_root, "anns_imgs_gcc_val_majorities_regions_20221101.json"
)
with open(anns_path, "r") as j:
    anns = json.loads(j.read())

img_pickle_path = os.path.join(
    args.feature_root,
    "IR_img_feature_PHASE_" + args.pre_train + "_" + args.model_name + ".pkl",
)
with open(img_pickle_path, "rb") as handle:
    img_feature = pickle.load(handle)

cap_pickle_path = os.path.join(
    args.feature_root,
    "IR_txt_feature_PHASE_" + args.pre_train + "_" + args.model_name + ".pkl",
)
