...
from clip_functions import (
    balance_on_characteristic,
    balance_on_intersection,
    get_text_features,
)
import argparse
import os
...
if args.from_pretrain in ["openai", "cc12m", "laion400m_e32"]:
    ...
    model_path = os.path.join(
        args.model_path + "cc3m_mix_" + args.from_pretrain,
        "epoch_" + str(args.epoch) + ".pt",
    )
...
ages_ = [
    f"a photo of a person between {tup[0]} and {tup[1]} years old" for tup in age_tuples
]
ages_.append("a photo of a person 70 or more years old")

races_ = [
    f"a photo of {race_} person"
    for race_ in [
        "an East Asian",
        "an Indian",
        "a Southeast Asian",
...
with torch.no_grad():
    text_features = (
        get_text_features(["a photo of a person"], model, tokenizer)
        .cpu()
        .numpy()
        .squeeze()
    )
...
for prompt in races_:
    ...
    with torch.no_grad():
        text_features = (
            get_text_features([prompt], model, tokenizer).cpu().numpy().squeeze()
        )
        text_features = text_features / np.linalg.norm(text_features)
...
race_df = pd.DataFrame(
    np.array(race_similarities).T, index=image_list, columns=["Person"] + races
)
race_df.to_csv(
    output_root + f"race_similarities_" + task_name + "_" + args.model + ".csv"
)
...
gender_df = pd.DataFrame(
    np.array(gender_similarities).T, index=image_list, columns=["Person"] + genders
)
gender_df.to_csv(
    output_root + f"gender_similarities_" + task_name + "_" + args.model + ".csv"
)
...
binary_gender_preferences = [
    1 if pref == "Person" else 0 for pref in gender_preferences
]
gender_df = pd.DataFrame(
    binary_gender_preferences, index=image_list, columns=["Gender"]
)
...
age_df = pd.DataFrame(
    np.array(age_similarities).T, index=image_list, columns=["Person"] + ages
)
age_df.to_csv(
    output_root + f"age_similarities_" + task_name + "_" + args.model + ".csv"
)
...
joint_df.to_csv(
    output_root + f"clip_preferences_" + task_name + "_" + args.model + ".csv"
)
print(joint_df)
...
for iter in range(ITERS):
    for race in races:
        balanced_df, balanced_preferences = balance_on_characteristic(
            population_df, joint_df, "race"
        )  # Balance dataset
        race_idx = balanced_df[
            balanced_df.race.isin([race])
        ]  # Sample only from target race or ethnicity
        prefs = balanced_preferences.loc[
            race_idx.index.str.split("/").str[1].str.split(".").str[0]
        ]
        pref_list = prefs["Race"].tolist()
        pct = sum(pref_list) / len(
            pref_list
        )  # Get percentage of images for which "person" is highest probability
...
for iter in range(ITERS):
    for gender in genders:
        balanced_df, balanced_preferences = balance_on_characteristic(
            population_df, joint_df, "gender"
        )  # Balance dataset
        gender_idx = balanced_df[
            balanced_df.gender.isin([gender])
        ]  # Sample only from target gender
        prefs = balanced_preferences.loc[
            gender_idx.index.str.split("/").str[1].str.split(".").str[0]
        ]
...
for iter in range(ITERS):
    for age in age_labels:
        balanced_df, balanced_preferences = balance_on_characteristic(
            population_df, joint_df, "age"
        )  # Balance dataset
        balanced_df, balanced_preferences = balance_on_characteristic(
            population_df, joint_df, "age"
        )  # Balance dataset
        age_idx = balanced_df[
            balanced_df.age.isin([age])
        ]  # Sample only from target age
        prefs = balanced_preferences.loc[
            age_idx.index.str.split("/").str[1].str.split(".").str[0]
        ]
...
balanced_df, balanced_preferences = balance_on_intersection(
    population_df, joint_df, ["gender", "age"]
)
intersections = list(set(balanced_df["intersection"].tolist()))
pref_dict_age, pref_dict_gender, pref_dict_race = (
...
for iter in range(ITERS):
    for intersection in intersections:
        ...
        prefs = balanced_preferences.loc[
            intersection_idx.index.str.split("/").str[1].str.split(".").str[0]
        ]
...
print(
    " ".join([f"({idx+1},{female_age_series[idx]})" for idx in range(len(age_labels))])
)

print(
    " ".join([f"({idx+1},{male_gender_series[idx]})" for idx in range(len(age_labels))])
)
print(
    " ".join(
        [f"({idx+1},{female_gender_series[idx]})" for idx in range(len(age_labels))]
    )
)

print(
    " ".join([f"({idx+1},{male_race_series[idx]})" for idx in range(len(age_labels))])
)
print(
    " ".join([f"({idx+1},{female_race_series[idx]})" for idx in range(len(age_labels))])
)
...
balanced_df, balanced_preferences = balance_on_intersection(
    population_df, joint_df, ["race", "gender", "age"]
)
intersections = list(set(balanced_df["intersection"].tolist()))
pref_dict_age, pref_dict_gender, pref_dict_race = (
    {intersection: [] for intersection in intersections},
...
wm_df_idx = population_df[
    population_df.race.isin(["White"]) & population_df.gender.isin(["Male"])
]
bf_df_idx = population_df[
    population_df.race.isin(["Black"]) & population_df.gender.isin(["Female"])
]
...
print(
    " ".join(
        [f'({idx+1},{wm_mean_dict["Race"][idx]*100})' for idx in range(len(age_labels))]
    )
)
print(
    " ".join(
        [f'({idx+1},{bf_mean_dict["Race"][idx]*100})' for idx in range(len(age_labels))]
    )
)
...
print(
    " ".join(
        [
            f'({idx+1},{wm_mean_dict["Gender"][idx]*100})'
            for idx in range(len(age_labels))
        ]
    )
)
print(
    " ".join(
        [
            f'({idx+1},{bf_mean_dict["Gender"][idx]*100})'
            for idx in range(len(age_labels))
        ]
    )
)
...
print(
    " ".join(
        [f'({idx+1},{wm_mean_dict["Age"][idx]*100})' for idx in range(len(age_labels))]
    )
)
print(
    " ".join(
        [f'({idx+1},{bf_mean_dict["Age"][idx]*100})' for idx in range(len(age_labels))]
    )
)
